...
def Analyse(t,weighting):
    ...
    lepcuts = "lep_n == 2 && lep_charge[0]*lep_charge[1] == -1 && lep_type[0] == 11 && lep_type[1] == 11"
    etacuts = "lep_eta[0] >=-2.4 && lep_eta[0] <=2.4 && lep_eta[1] >=-2.4 && lep_eta[1] <=2.4"
    ptcuts = "lep_pt[0] >= 30e3 && lep_pt[1] >= 30e3" # && lep_pt[0] <= 70e3 && lep_pt[1] <= 70e3
    masscuts = "Mll >= 66e3" # && Mll <= 112e3
    conecuts = "lep_ptcone30[0] < 4000 && lep_ptcone30[1] < 4000 && lep_etcone20[0] <= 5000 && lep_etcone20[1] <= 5000"
    multicuts = "(" + lepcuts + "&&" + etacuts + "&&"+ ptcuts + "&&" + conecuts + "&&" + masscuts + ")" #
